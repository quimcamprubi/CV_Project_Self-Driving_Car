...
def perspective_warp(original, img, dst_size):
    ...
    src=np.float32([(0.43,0.65),(0.58,0.65),(0.1,1),(1,1)]) * np.float32(img_size)
    dst=np.float32([(0,0), (1, 0), (0,1), (1,1)]) * np.float32(dst_size)
    """src1 = (120, 200)
    src2 = (38, 255)
    src3 = (460, 255)
    src4 = (380, 200)
...
def inv_perspective_warp(img, dst_size):
    ...
    dst=np.float32([(0.43,0.65),(0.58,0.65),(0.1,1),(1,1)]) * np.float32(dst_size)
    src=np.float32([(0,0), (1, 0), (0,1), (1,1)]) * np.float32(img_size)
...
if __name__ == "__main__":
    ...
    while (video.isOpened()):
        ...
        fontScale = 1
        fontColor = (0,0,255)
        cv2.putText(img, 'Offset: {:.4f}'.format(offset), (570, 650), font, fontScale, fontColor, 2)
        
        cv2.imshow('Lane detection', img)
        t_frame = time.time() - start
        print("Execution time for one frame =", t_frame, "seconds.")
        if (t_frame < threshold_t):
            wait = threshold_t - t_frame
